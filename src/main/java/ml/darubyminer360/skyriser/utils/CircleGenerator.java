...
public class CircleGenerator {
	final static int MAX_SQUARE = 10000; // The maximum radius whose square the generator will cache.

	static final double SQUARE_ROOT_OF_TWO = 1.4142135624;

	// Cache circle location sets by radius and whether enclosed locations are allowed

	static HashMap<Integer, HashSet<RelativeLocation>> cacheXZEnclosed = new HashMap<>();

	static HashMap<Integer, HashSet<RelativeLocation>> cacheXYEnclosed = new HashMap<>();

	static HashMap<Integer, HashSet<RelativeLocation>> cacheZYEnclosed = new HashMap<>();

	static HashMap<Integer, HashSet<RelativeLocation>> cacheXZIgnoreEnclosed = new HashMap<>();

	static HashMap<Integer, HashSet<RelativeLocation>> cacheXYIgnoreEnclosed = new HashMap<>();

	static HashMap<Integer, HashSet<RelativeLocation>> cacheZYIgnoreEnclosed = new HashMap<>();

	static HashMap<Integer, Integer> squareRootCache = new HashMap<>();
	static {
	...
	/**
	 * Generates a set of locations that represent a rasterized sphere. Uses cached
	 ...
	 *                       the sphere's plane
	 * @param allowBurrs - whether the sphere should have 'burrs' or single blocks jutting out at the tips of the axes
	 * @return a set of locations representing a rasterized sphere
	 * @throws IllegalArgumentException if center point is null, plane is null, or
	 *                                  radius is less than zero
	 */
	public static HashSet<Location> generateSphere(Location center, int radius, boolean ignoreEnclosed, boolean allowBurrs, boolean hollow) {
	...
	public static HashSet<Location> generateCircle(Location center, int radius, Plane plane, boolean ignoreEnclosed, boolean allowBurrs, boolean hollow) {
		...
		if (whichCacheToUse.containsKey(radius)) { // optimized caching return
			...
			case XZ -> {
				...
				if (ignoreEnclosed) {
					...
					while (iteratorXZ.hasNext()) {
						...
						if (locs.contains(new RelativeLocation(rl.rX + 1, rl.rY, rl.rZ)) && locs.contains(new RelativeLocation(rl.rX, rl.rY, rl.rZ + 1))) {
							iteratorXZ.remove();
						}
					}
				}
			...
			case XY -> {
				...
				if (ignoreEnclosed) {
					...
					while (iteratorXY.hasNext()) {
						...
						if (locs.contains(new RelativeLocation(rl.rX + 1, rl.rY, rl.rZ)) && locs.contains(new RelativeLocation(rl.rX, rl.rY + 1, rl.rZ))) {
							iteratorXY.remove();
						}
					}
				}
		...
		if (!allowBurrs) {
			return generateCircle(center, radius, plane, ignoreEnclosed, allowBurrs, hollow);
		}

		for (RelativeLocation rl : locs)
			set.add(new Location(center.getWorld(), center.getBlockX() + rl.rX, center.getBlockY() + rl.rY, center.getBlockZ() + rl.rZ));
		if (!hollow)
			set.addAll(generateCircle(center, radius - 1, plane, ignoreEnclosed, allowBurrs, hollow));
		return set;
	}
	 ...
	 */
	static void fillOctantsXZ(Set<RelativeLocation> set, RelativeLocation location) {
	 ...
	 * @param location - location in octant 2 to be cloned
	 */
	static void fillOctantsZY(Set<RelativeLocation> set, RelativeLocation location) {
	 ...
	 * @param location - location in octant 2 to be cloned
	 */
	static void fillOctantsXY(Set<RelativeLocation> set, RelativeLocation location) {
